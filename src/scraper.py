...
class Scraper():
    ...
    def get_tweets(self):
        ...
        for day_index in range(number_of_days, -1, -1):
            ...
            #print((current_day_in_week - today).days)
            self.search_tweets = self.api.search_tweets(q = self.search, lang = "en", count = self.count, tweet_mode = "extended", until = current_day_in_week.date(), since_id = oldest_id)
            
          #  for tweet in self.search_tweets:
           #     print(tweet.created_at)
            if len(self.search_tweets) > 0:
                oldest_id = self.search_tweets[0].id
            #self.search_tweets.clear()

    def get_tweets_old(self):
        if self.timeline == "new":
            #oldest_id = self.tweet_dao.get_tweet_id("MAX")
            #, since_id = oldest_id[0]
           # print("Newest: " + str(oldest_id[0]))
            self.search_tweets = self.api.search_tweets(q = self.search, lang = "en", count = self.count, tweet_mode="extended")
        elif self.timeline == "old":
            newest_id = self.tweet_dao.get_tweet_id("MAX")
            print("Oldest: " + str(newest_id))
            self.search_tweets =self.api.search_tweets(q = self.search, lang = "en", count = self.count, tweet_mode="extended", since_id = newest_id[0])
    ...
    def start(self):
      ...
      #  print(self.company_name + " thread complete.")
...
##            print(f"{tweet.user.name}:{tweet.text}"
       ...
       # print(self.search)
        ...
        self.tweet_dao.add_to_database(tweet.Tweet(search_tweet.id, self.company_name, search_tweet.full_text, lemmatized_sentence, search_tweet.created_at))
